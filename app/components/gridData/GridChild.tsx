...
const GridChild: React.FC<ChildComponentProps> = ({ childName, childId, childContainerLink, chipsOrder, tagsOrder, h1Order, h2Order, h3Order, h4Order, paragraphOrder, assetOrder, checkboxListOrder, socialMediaOrder, buttonOrder, customHeaderCss, modalTriggerOrder }) => {
                      ...
                      {currentElement.Asset &&
                        currentElement.Asset.map((assetContentsData) => (
                          (assetContentsData.imageSrc || assetContentsData.videoSrc) && (
                            <div key={assetContentsData.id} className={`order-${assetOrder} asset overflow-hidden`}>
                              ...
                              {currentElement.SocialMedia.map((socialMediaData) => (
                                socialMediaData.mediaLink && (
                                  <Link key={socialMediaData.id} href={socialMediaData.mediaLink}>
                                    <div className='w-10 h-10 mr-4 items-center socialMediaIconContainer rounded-md'>
                                      <img src={socialMediaData.mediaIcon} className='w-full h-full object-contain' alt='Social Media Icon' />
                                    </div>
                                  </Link>
                                )
